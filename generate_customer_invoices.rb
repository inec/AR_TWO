...
#puts customers_with_orders[1].inspect
#customer=customers_with_orders[1]
customers_with_orders.each do |customer|
#puts customer.inspect
#puts customer.orders.count
#new_order=customer.orders[0]
...
customer.orders.each do |new_order|
  ...
  new_order.line_items.each do |line_item|
    ...
    puts "#{item_name} ... #{item_quantity} x #{currency(item_price)} = #{currency(item_total)}"
